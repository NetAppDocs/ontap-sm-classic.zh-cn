...
summary: 您可以使用 System Manager 手动创建启用了 FabricPool 的聚合，也可以通过将云层附加到 SSD 聚合将现有 SSD 聚合转换为启用了 FabricPool 的聚合。 
---
...
[role="lead"]
您可以使用 System Manager 手动创建启用了 FabricPool 的聚合，也可以通过将云层附加到 SSD 聚合将现有 SSD 聚合转换为启用了 FabricPool 的聚合。
