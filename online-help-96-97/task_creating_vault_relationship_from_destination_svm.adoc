...
---
= 使用 System Manager - ONTAP 9.7 及更早版本从目标 SVM 创建存储关系
:allow-uri-read: 


[role="lead"]
您可以使用 ONTAP System Manager classic （适用于 ONTAP 9.7 及更早版本）从目标 Storage Virtual Machine （ SVM ）创建存储关系，并分配存储策略以创建备份存储。如果系统上发生数据丢失或损坏，可以从备份存储目标还原备份的数据。
