...
[role="lead"]
ONTAP System Manager classic (适用于ONTAP 9.7及更早版本)会将名称服务配置信息存储在一个表中、该表相当于UNIX系统上的`/etc/nsswitch.conf`文件。您必须了解该表的功能以及 ONTAP 如何使用它，以便可以根据您的环境对其进行适当配置。
...
xref:task_editing_svm_settings.adoc[编辑 SVM 设置]
