...
* 如果要在已配置 FC 的 SVM 上配置卷，则必须验证 FC 服务是否正在运行。
+
xref:task_verifying_that_fc_service_is_running_on_existing_svm.adoc[验证 FC 服务是否正在现有 SVM 上运行]

* 如果要在已启用 FC 但尚未配置 FC 的现有 SVM 上配置卷，请在现有 SVM 上配置 iSCSI 。
+
xref:task_configuring_iscsi_fc_creating_lun_on_existing_svm.adoc[在现有 SVM 上配置 FC]
